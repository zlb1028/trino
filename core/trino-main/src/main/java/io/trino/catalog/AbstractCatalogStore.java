...
import io.trino.spi.filesystem.SecurePathWhiteList;
...
import static io.trino.catalog.CatalogFilePath.getCatalogBasePath;
        ...
        implements CatalogStore
{
    ...
    protected final FileSystemClient fileSystemClient;
    private final int maxFileSizeInBytes;

    public AbstractCatalogStore(String baseDirectory, FileSystemClient fileSystemClient, int maxFileSizeInBytes)
    {
        ...
        if (!fileSystemClient.exists(getCatalogBasePath(baseDirectory))) {
            try {
                ...
                throw new TrinoException(GENERIC_INTERNAL_ERROR, "Failed to prepare base directory.", e);
            }
        }
    }
