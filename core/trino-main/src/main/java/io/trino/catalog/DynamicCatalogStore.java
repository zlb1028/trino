...
public class DynamicCatalogStore
{
     ...
     */
    private void loadLocalCatalog(CatalogStore localCatalogStore, CatalogInfo catalogInfo, io.trino.catalog.CatalogFileInputStream configFiles)
    {
        ...
        try {
            ...
            connectorManager.createCatalog(catalogName, catalogInfo.getConnectorName(), ImmutableMap.copyOf(properties));
            ...
            throw new TrinoException(GENERIC_INTERNAL_ERROR, throwMessage);
        }
    }
    ...
    private void unloadLocalCatalog(CatalogStore localCatalogStore, String catalogName)
    {
        ...
        try {
            ...
            throw new TrinoException(GENERIC_INTERNAL_ERROR, "Delete catalog failed.", e);
        }
    ...
    private void updateLocalCatalog(CatalogInfo catalogInfo, io.trino.catalog.CatalogFileInputStream configFiles)
    {
        String catalogName = catalogInfo.getCatalogName();
    ...
    public synchronized void loadCatalog(String catalogName)
    {
        ...
        try (io.trino.catalog.CatalogFileInputStream configFiles = shareCatalogStore.getCatalogFiles(catalogName)) {
        ...
        catch (IOException | TrinoException ex) {
            throw new TrinoException(GENERIC_INTERNAL_ERROR, ex.getMessage(), ex);
        }
    }
     ...
     */
    public synchronized void loadCatalogAndCreateShareFiles(CatalogInfo catalogInfo, io.trino.catalog.CatalogFileInputStream configFiles)
    {
        String catalogName = catalogInfo.getCatalogName();
        try {
            ...
            unloadCatalog(catalogName);
            throw new TrinoException(GENERIC_USER_ERROR, ex.getMessage(), ex);
        }
    }
    ...
    private void waitForAllNodeDeletedCatalog(String catalogName)
    {
        ...
        try {
            ...
            throw new TrinoException(GENERIC_INTERNAL_ERROR, "Wait all nodes unload catalog, but timeout");
        }
    }

    private void updateRemoteCatalog(CatalogInfo catalogInfo, io.trino.catalog.CatalogFileInputStream configFiles)
    {
        String catalogName = catalogInfo.getCatalogName();
        // delete catalog files from share file system.
        try {
        ...
        try {
            ...
            throw new TrinoException(GENERIC_INTERNAL_ERROR, "Store new catalog files to share file system failed");
        }
        // wait for at least one node added catalog.
        waitForAtLeastOneNodeAddedCatalog(catalogName);
    }

    public synchronized void updateCatalogAndShareFiles(CatalogInfo catalogInfo, io.trino.catalog.CatalogFileInputStream configFiles)
    {
        String catalogName = catalogInfo.getCatalogName();
        CatalogInfo previousCatalogInfo;
        io.trino.catalog.CatalogFileInputStream previousCatalogFiles;
        ...
        try {
            ...
            throw new TrinoException(GENERIC_INTERNAL_ERROR, "Backup previous catalog information and files failed");
        }
        ...
        // prepare new files.
        io.trino.catalog.CatalogFileInputStream.Builder builder = new io.trino.catalog.CatalogFileInputStream.Builder((int) dynamicCatalogConfig.getCatalogMaxFileSize().toBytes());

        log.info("-- Try to updating catalog [%s] to version [%s] --", catalogName, catalogInfo.getVersion());
        previousCatalogFiles.mark();
        try (io.trino.catalog.CatalogFileInputStream newCatalogFiles = builder
        ...
        catch (TrinoException | IOException ex) {
