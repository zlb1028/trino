...
import static io.trino.catalog.CatalogFileInputStream.CatalogFileType.CATALOG_FILE;
import static io.trino.catalog.CatalogFileInputStream.CatalogFileType.GLOBAL_FILE;
...
public class CatalogResource
{
    ...
    @Inject
    public CatalogResource(GroupProvider groupProvider, DynamicCatalogService service, DynamicCatalogConfig config)
    {
                                  ...
                                  @Context HttpServletRequest servletRequest)
    {
        ...
        try (CatalogFileInputStream configFiles = toCatalogFiles(catalogConfigFileBodyParts, globalConfigFilesBodyParts)) {
            return service.createCatalog(catalogInfo,
                    configFiles,
                    new HttpRequestSessionContext(headers, Optional.of(""), remoteAddress, identity, groupProvider));
        }
                                ...
                                @Context HttpServletRequest servletRequest)
    {
        ...
        try {
            response = service.dropCatalog(catalogName, new HttpRequestSessionContext(headers, Optional.of(""), remoteAddress, identity, groupProvider));
        }
        catch (WebApplicationException ex) {
            throw ex;
        }
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Response showCatalogs(@Context HttpHeaders httpHeaders,
                                 @Context HttpServletRequest servletRequest)
    {
        Response response;
        String remoteAddress = servletRequest.getRemoteAddr();
        ...
        MultivaluedMap<String, String> headers = httpHeaders.getRequestHeaders();
        try {
            response = service.showCatalogs(new HttpRequestSessionContext(headers, Optional.of(""), remoteAddress, identity, groupProvider));
        }
        catch (WebApplicationException ex) {
            throw ex;
        }
