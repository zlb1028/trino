        ...
        implements SecurityKeyManager
{
            ...
            throws SecurityKeyException
    {
        ...
        try (FileSystemClient fileSystemClient =
                fileSystemClientManager.getFileSystemClient(config.getShareFileSystemProfile(), Paths.get("/"))) {
            boolean isStoreFileExists = fileSystemClient.exists(keystorPath);
            KeyStore keyStore = KeyStore.getInstance(PKCS12);
    ...
    private void createStoreDirIfNotExists()
    {
        ...
        try (FileSystemClient fileSystemClient =
            ...
            if (fileSystemClient.exists(Paths.get(tmpFileDir))) {
                return;
            }

            fileSystemClient.createDirectories(Paths.get(tmpFileDir));
