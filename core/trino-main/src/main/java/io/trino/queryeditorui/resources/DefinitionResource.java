...
public class DefinitionResource
{
    ...
    public Response dropSchema(@FormDataParam("catalog") String catalog,
                               @FormDataParam("schema") String schema,
                               @Context HttpHeaders httpHeaders,
                               @Context HttpServletRequest servletRequest)
    {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("table/{table}")
    public Response deleteTable(@FormDataParam("tableInfo") String tableInfoJson,
                                 @Context HttpHeaders httpHeaders,
                                 @Context HttpServletRequest servletRequest)
    {
        String user = AccessControlUtil.getUser(accessControl, groupProvider, httpHeaders, servletRequest);
        tableService.dropTable(toTableInfo(tableInfoJson), user);
        return Response.ok().build();
    }
