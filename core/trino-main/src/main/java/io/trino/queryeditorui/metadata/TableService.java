...
public class TableService
{
    ...
    public void dropTable(String catalog, String schema, String table, String user)
    {
        QueryRunner queryRunner = queryRunnerFactory.create(QueryEditorUIModule.UI_QUERY_SOURCE, user);
        String statement = String.format("DROP TABLE IF EXISTS %s.%s.%s", catalog, schema, table);
        Boolean result = StatementResultUtil.definitionStatement(queryRunner, statement);
        if (result == null || !result) {
            throw new IllegalArgumentException(String.format("failed to drop table: [%s.%s.%s], please check you arguments", catalog, schema, table));
        }
    }
